...
@section('header', 'Add Video Tutorial')
        ...
        <form action="{{ route('admin.videos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="url" name="video_url" value="{{ old('video_url') }}" required placeholder="https://www.youtube.com/watch?v=..." class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500">
                </div>
                <div>
                    <label class="block text-xs uppercase font-bold text-slate-400 mb-2">Custom Thumbnail (Optional)</label>
            ...
            <div class="pt-4 border-t border-slate-800">
                ...
                <div class="space-y-4">
                    <input type="text" name="seo_title" placeholder="Meta Title for Google Video Search" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl shadow-lg transition-all">Save Video</button>
            </div>
        </form>
    </div>
</div>
