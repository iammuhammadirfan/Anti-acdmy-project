...
@section('page_title', 'Create Role & Assign Permissions')
...
<div class="max-w-4xl space-y-6">
    ...
    <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-4">
            <h3 class="font-bold text-slate-900 text-base">Role Information</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Role Name *</label>
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Appointment Manager" required
                ...
                <div>
                    ...
                    <input type="text" name="description" value="{{ old('description') }}" placeholder="Handles calendar and appointments"
                           class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:bg-white transition">
                </div>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-slate-50/50">
                                ...
                                @foreach($actions as $action)
                                    <td class="py-3 px-4 text-center">
                                        <input type="checkbox" name="permissions[{{ $key }}][{{ $action }}]" value="1" class="rounded text-brand-600">
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm px-6 py-2.5 rounded-xl shadow-md shadow-brand-500/20 transition">Create Role</button>
        </div>
    </form>
</div>
@endsection
