        ...
        <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $blog->title) }}" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500">
                        ...
                        <option value="review" {{ $blog->status === 'review' ? 'selected' : '' }}>Needs Review</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs text-white focus:outline-none focus:border-blue-500">{{ old('excerpt', $blog->excerpt) }}</textarea>
            </div>

            <div>
                <label class="block text-xs uppercase font-bold text-slate-400 mb-2">Full Article Body (HTML / Markdown supported) *</label>
                <textarea name="content" rows="8" required class="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-xs font-mono text-white focus:outline-none focus:border-blue-500">{{ old('content', $blog->content) }}</textarea>
            </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($tags as $t)
                            <label class="inline-flex items-center gap-1.5 text-xs text-slate-300 bg-slate-950 border border-slate-800 px-3 py-1.5 rounded-lg cursor-pointer hover:border-slate-700">
                                <input type="checkbox" name="tags[]" value="{{ $t->id }}" {{ in_array($t->id, $selectedTags) ? 'checked' : '' }} class="rounded bg-slate-900 border-slate-700 text-blue-600 focus:ring-0">
                                {{ $t->name }}
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="pt-4 border-t border-slate-800">
                ...
                <div class="space-y-4">
                    <input type="text" name="seo_title" value="{{ old('seo_title', $blog->seo_title) }}" placeholder="SEO Title tag" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500">
                    <textarea name="seo_description" rows="2" placeholder="Meta description..." class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-blue-500">{{ old('seo_description', $blog->seo_description) }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl shadow-lg transition-all">Update Article</button>
            </div>
        </form>
    </div>
</div>
