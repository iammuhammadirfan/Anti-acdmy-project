...
@section('page_title', 'Create User & Assign Permissions')
...
<div class="max-w-4xl space-y-6">
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
                ...
                <div class="flex flex-wrap gap-4">
                    @foreach($roles as $role)
                        <label class="flex items-center gap-2 cursor-pointer bg-slate-50 px-3.5 py-2 rounded-xl border border-slate-200 text-sm">
                            <input type="checkbox" name="roles[]" value="{{ $role->id }}" class="rounded text-brand-600 focus:ring-brand-500">
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm px-6 py-2.5 rounded-xl shadow-md shadow-brand-500/20 transition">Create Staff Account</button>
        </div>
    </form>
</div>
@endsection
